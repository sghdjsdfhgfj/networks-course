...
with open(blacklist_name) as file:
...
def process_request(conn):
    ...
    if host in blacklist:
        print(f'request {method} {path}: {host} is blacklisted')
        status_code = HTTPStatus(403)
        response_body = "page is blacklisted".encode()
    elif method == 'GET' and path in cache and body in cache[path]:
